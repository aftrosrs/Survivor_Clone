...
func change_time():
	...
	var h = int(time/60.0)/60.0# sets it up so i can have an hours.
...
func adjust_gui_collection(upgrade):
	...
	if get_type != "item":# if it isnt an "item"
		var get_collected_displaynames = []#adds array for us to hold our collected display names
		for i in collected_upgrades:# loop through all of our already collected upgrades
			get_collected_displaynames.append(UpgradeDb.UPGRADES[i]["displayname"])#displays  the name
		if not get_upgraded_displayname in get_collected_displaynames:# check if its in that array
			var new_item = ITEM_CONTAINER.instantiate()
			new_item.upgrade = upgrade#set the upgrade value for the icon
			match get_type:#checks the type 
				"weapon":
					Collected_Weapons.add_child(new_item)#add to collected weapons node
				"upgrade":
					Collected_Upgrades.add_child(new_item)#add to collected upgrades node
